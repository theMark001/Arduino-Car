...
void GMotor::setSpeed(int16_t duty) {
    if (_mode < 2) {
        ...
        // Fix standard analogWrite(pin, 255) for >8 bit
        if (_maxDuty > 255 && abs(_duty) == 255) _duty++;
        
        if (duty == 0) run(STOP, 0);
        else {
            if (duty > 0) {
                if (_minDuty != 0) _duty = _duty * _k + _minDuty;	// Compress range
                run(_mode, _duty);
            } else {
                if (_minDuty != 0) _duty = _duty * _k - _minDuty;	// Compress range
                run(BACKWARD, -_duty);
...
void GMotor::run(GM_workMode mode, int16_t duty) {
    ...
    if (_deadtime > 0 && _lastMode != mode) {
        _lastMode = mode;
        setPins(_level, _level, 0);	// turn everything off
